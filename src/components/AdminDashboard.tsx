...
export const AdminDashboard = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data) {
        setAllowRegistrations(data.value === true || data.value === "true"); // Garante conversão para booleano
      } else {
        // Se não existir, define como false por padrão e cria o registro
        await supabase
          .from("system_settings")
          .insert({
      ...
      setAllowRegistrations(false); // Fallback seguro
    }
  };
    ...
    try {
      ...
      if (aggregatedData) {
        aggregatedData.forEach((record) => {
          ...
          totalOfferings += (parseFloat(String(record.offering_cash || 0)) + parseFloat(String(reg.offering_pix || 0)));
        });
      }
      setStats({
